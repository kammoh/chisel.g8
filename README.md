# chisel.g8

A minimal [Chisel] template [Giter8](http://www.foundweekends.org/giter8/) template using [Mill](https://mill-build.org/mill/index.html) and
based on [com-lihaoyi/mill-scala-hello.g8](https://github.com/com-lihaoyi/mill-scala-hello.g8).

## Getting Started

Usage with [Giter8](https://mill-build.org/mill/scalalib/intro.html):

```sh
g8 kammoh/chisel.g8
```

_Usage with Mill_

```sh
mill init kammoh/chisel.g8
```

You can also specify the project's name and directory:

```sh
mill init file:///Volumes/src/chisel.g8 --name=myProject -o my-project
```
